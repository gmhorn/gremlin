use crate::{spectrum::Sampled, Real};
...
mod consts {
    use crate::spectrum::Sampled64;

    const CIE_X: Sampled64 = Sampled64::from([0.001368, 0.002236, 0.004243, 0.007650, 0.014310, 0.023190, 0.043510,
	...
	0.000117, 0.000083, 0.000059]);
}

impl XYZ {
    pub fn from_sampled<R: Real>(s: Sampled<R>) -> Self {
        let v: f32 = s.iter().map(|x| x.to_f32().unwrap()).fold(1.0, |acc, x| acc * x);
    }
}
...
mod tests {
    ...
    fn convert() {
        let big: [f64; 4] = [0.0; 4];
        let small: [f32; 4] = [0.0; 4];

        let small: [f32; 4] = big.map(|x| x as f32);
    }
}